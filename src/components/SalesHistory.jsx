...
import { DEFAULT_LAYOUT } from '@/components/InvoiceLayoutEditor';
...
function SalesHistory() {
    ...
    const handleUpdate = () => {
        ...
        const updatedTransactions = savedTransactions.map(t => {
            if (t.id === editingId) {
                return {
                    ...
                    clientName: editForm.clientName, // Aseguramos que el nombre en la tabla cambie
                    amount: editForm.total,
                    paymentMethod: editForm.paymentMethod, // SINCRO CRÍTICA: Aseguramos que el método se actualice
                    type: 'income' // Aseguramos que sea ingreso
                };
            }
            return t;
        });
        ...
        setEditingId(null);
        setViewMode('list');
        toast({ title: "Venta actualizada", description: "Se ajustaron los productos, el total y el flujo de caja correctamente." });

        // RECARGA PARA FORZAR SINCRONIZACIÓN DEL CONTEXTO
        setTimeout(() => window.location.reload(), 500);
    };

    const handleReprint = async (sale) => {
        try {
            toast({ title: "Generando PDF...", description: "Reconstruyendo comprobante." });

            // Reconstruct form data to match what InvoicePDF expects
            const formData = {
                clientName: sale.clientName || 'CONSUMIDOR FINAL',
                ruc: sale.ruc || '0',
                invoiceDate: sale.date,
                paymentMethod: sale.paymentMethod,
                invoiceNumber: sale.type === 'TICKET' ? 'TKT-REIMPR' : 'INV-REIMPR',
                vatRate: '10' // Default assumption
            };

            const items = sale.items || [];

            // Generate PDF Blob
            const savedLayout = JSON.parse(localStorage.getItem('invoiceLayoutCoordsV2') || 'null');
            ...
            toast({
                ...
                description: "El comprobante se ha abierto para imprimir.",
            });
        } catch (error) {
            console.error("Error reprinting PDF:", error);
            toast({
                title: "Error de Impresión",
                description: "No se pudo regenerar el comprobante.",
                variant: "destructive"
            });
        }
    };
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex justify-center items-center gap-2">
                                                        {isEditing ? (
                                                            ...
                                                            <>
                                                                <button onClick={() => startEdit(sale)} className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100" title="Editar">
                                                                    <Edit2 className="w-4 h-4" />
                                                                </button>
                                                                <button onClick={() => handleReprint(sale)} className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100" title="Reimprimir PDF">
                                                                    <Printer className="w-4 h-4" />
                                                                </button>
                                                                <button onClick={() => deleteSale(sale.id)} className="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100" title="Eliminar">
                                                                    <Trash2 className="w-4 h-4" />
                                                                </button>
                                                            </>
                                                        )}
