...
import { Shield, Database, Save, HardDrive, CheckCircle, AlertTriangle, RotateCcw, CloudUpload, Download } from 'lucide-react';
...
import { supabase } from '@/lib/customSupabaseClient'; // Needed for direct restore
...
const SystemMaintenance = () => {
    ...
    const handleSyncSalesWithCash = () => {
        try {
            ...
            const updatedTransactions = transactions.map(tx => {
                // Find matching sale by ID
                const matchingSale = sales.find(s => s.id === tx.id);
                if (matchingSale && matchingSale.paymentMethod !== tx.paymentMethod) {
                    updatedCount++;
                    return {
                        ...tx,
                        paymentMethod: matchingSale.paymentMethod
                    };
                }
                return tx;
            });
            ...
            if (updatedCount > 0) {
                ...
                toast({
                    ...
                    description: `Se han corregido ${updatedCount} movimientos de caja.`,
                    variant: "success"
                });
                // Reload page to reflect changes in other components if needed
                setTimeout(() => window.location.reload(), 2000);
            } else {
                toast({
                    title: "Todo en orden",
                    description: "No se encontraron discrepancias entre ventas y caja.",
                });
            }
        } catch (error) {
            console.error(error);
    ...
    const handleFullExport = () => {
        const data = {
            ...
            cashOpeningHistory: JSON.parse(localStorage.getItem('cashOpeningHistory') || '[]'),
            cashClosingHistory: JSON.parse(localStorage.getItem('cashClosingHistory') || '[]'),
            invoiceLayoutCoordsV2: JSON.parse(localStorage.getItem('invoiceLayoutCoordsV2') || '{}'),
            clients: JSON.parse(localStorage.getItem('clients') || '[]'),
            invoiceFormData: JSON.parse(localStorage.getItem('invoiceFormData') || '{}')
        };
        ...
        toast({
            ...
            description: "Guarda este archivo en un lugar seguro. Contiene TODOS tus datos.",
            variant: "success"
        });
    };
    ...
    const handleImportBackup = async (event) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                toast({ title: "Iniciando Restauración", description: "Subiendo datos a la nueva base de datos..." });
                ...
                if (data.products && data.products.length > 0) {
                    ...
                    await supabase.from('productos').upsert(productsPayload, { onConflict: 'codigo' });
                }

                // 2. Restore Clients
                if (data.clients && data.clients.length > 0) {
                    // Filter valid RUCs
                    const validClients = data.clients.filter(c => c.ruc);
                    await supabase.from('clientes').upsert(validClients, { onConflict: 'ruc' });
                }

                // 3. Restore Sessions
                if (data.cashOpeningHistory && data.cashOpeningHistory.length > 0) {
                    const sessionsPayload = data.cashOpeningHistory.map(s => ({
                        id: s.id,
                    ...
                    await supabase.from('caja_sesiones').upsert(sessionsPayload, { onConflict: 'id' });
                }

                // 4. Restore Cash Movements
                if (data.cashTransactions && data.cashTransactions.length > 0) {
                    const movesPayload = data.cashTransactions.map(tx => ({
                        id: tx.id,
                    ...
                    await supabase.from('movimientos_caja').upsert(movesPayload, { onConflict: 'id' });
                }
                ...
                if (data.salesHistory && data.salesHistory.length > 0) {
                    for (const sale of data.salesHistory) {
                        ...
                        if (sale.items && sale.items.length > 0) {
                            // First clean old items if upserting
                            ...
                            await supabase.from('detalles_ventas').insert(itemsPayload);
                        }
                    }
                }

                // 6. Restore Invoice Layout (To Cloud Config)
                if (data.invoiceLayoutCoordsV2) {
                    await supabase.from('configuraciones').upsert({
                        key: 'invoiceLayoutCoordsV2',
                        value: data.invoiceLayoutCoordsV2
                    });
                    // Also keep local just in case
                    localStorage.setItem('invoiceLayoutCoordsV2', JSON.stringify(data.invoiceLayoutCoordsV2));
                }

                toast({ title: "Restauración Exitosa", description: "Todos los datos están en la Nube.", variant: "success" });

            } catch (error) {
                console.error("Import Error:", error);
                toast({ title: "Error", description: "Fallo al importar el archivo.", variant: "destructive" });
            }
        };
    ...
    const handleFactoryReset = () => {
        if (!confirm("¿ESTÁS SEGURO? Esto borrará tus datos LOCALES. Solo hazlo si ya restauraste los datos a la Nube.")) return;

        const layout = localStorage.getItem('invoiceLayoutCoordsV2'); // Preserve Layout
        localStorage.clear();
        if (layout) localStorage.setItem('invoiceLayoutCoordsV2', layout); // Restore Layout

        window.location.reload();
    };

    const handleDownloadFromCloud = async () => {
        if (!confirm("¿Descargar datos de la Nube? Esto sobreescribirá tus datos locales actuales.")) return;
        toast({ title: "Descargando...", description: "Recuperando datos de la nube..." });
        ...
        try {
            ...
            if (sales && details) {
                const mappedSales = sales.map(s => ({
                    ...
                    ruc: s.cliente_ruc,
                    type: s.tipo,
                    total: parseFloat(s.total), // Ensure number
                ...
                localStorage.setItem('salesHistory', JSON.stringify(mappedSales));
            }
            ...
            if (products) {
                const mappedProducts = products.map(p => ({
                    ...
                    stock: p.stock, // Keep as string or number depending on usage
                    um: p.unidad_medida,
                    cost: p.costo
                }));
            ...
            if (sessions) {
                ...
                localStorage.setItem('cashOpeningHistory', JSON.stringify(mappedSessions));
            }
            ...
            if (moves) {
                ...
                localStorage.setItem('cashTransactions', JSON.stringify(mappedMoves));
            }

            toast({ title: "Descarga Exitosa", description: "Datos actualizados desde la Nube.", variant: "success" });
            setTimeout(() => window.location.reload(), 1500);

        } catch (error) {
            console.error("Download Error:", error);
            toast({ title: "Error", description: "No se pudo descargar de la nube.", variant: "destructive" });
        }
    };
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
                {/* Sync Tool Card */}
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-orange-100 flex flex-col justify-between">
                    <div>
                        <div className="p-3 bg-orange-50 text-orange-600 rounded-2xl w-fit mb-6">
                            <RotateCcw className="w-6 h-6" />
                        </div>
                        <h3 className="text-xl font-black text-gray-900 mb-2 uppercase">Reparar Sincronización</h3>
                        <p className="text-sm text-gray-500 font-medium leading-relaxed italic border-l-2 border-orange-200 pl-4 mb-8">
                            Corrige errores en el Flujo de Caja. Útil si las ventas con Tarjeta aparecen como Efectivo en tu Arqueo.
                        </p>
                    </div>
                    <Button
                        onClick={handleSyncSalesWithCash}
                        className="w-full bg-orange-600 hover:bg-orange-700 text-white font-black uppercase tracking-widest py-6 rounded-xl shadow-lg shadow-orange-200 transition-all border-none"
                    >
                        Iniciar Reparación
                    </Button>
                </div>
            ...
            <div className="mt-8 border-t pt-8">
                <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                    ...
                    Zona de Peligro / Reconstrucción
                </h3>
                <div className="bg-red-50 p-6 rounded-xl border border-red-200">
                    <p className="text-red-800 mb-4 font-medium">
                        Paso 1: Antes de cualquier reestructuración, descarga una copia completa de tus datos locales.
                    </p>
                    <button
                        onClick={handleFullExport}
                        className="flex items-center gap-2 px-8 py-4 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold shadow-lg shadow-red-200 transition-all active:scale-95 w-full justify-center md:w-auto"
                    >
                        <Download className="w-5 h-5" />
                        DESCARGAR COPIA DE SEGURIDAD TOTAL (.JSON)
                    </button>
                    <p className="text-red-600 text-xs mt-2 text-center md:text-left">
                        * Guarda este archivo. Es la única forma de recuperar tus datos si borramos la base de datos.
                    </p>
                </div>

                <div className="bg-blue-50 p-6 rounded-xl border border-blue-200 mt-6">
                    <p className="text-blue-800 mb-4 font-medium">
                        Paso 2: Restaurar datos al Nuevo Sistema (Cloud).
                    </p>
                    <label className="flex items-center gap-2 px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold shadow-lg shadow-blue-200 transition-all active:scale-95 w-full justify-center md:w-auto cursor-pointer">
                        <CloudUpload className="w-5 h-5" />
                        SUBIR COPIA DE SEGURIDAD Y RESTAURAR
                        <input type="file" accept=".json" onChange={handleImportBackup} className="hidden" />
                    </label>
                </div>

                <div className="bg-gray-100 p-6 rounded-xl border border-gray-300 mt-6">
                    <p className="text-gray-800 mb-4 font-medium">
                        Paso 3: Limpiar Navegador (Factory Reset).
                    </p>
                    <button
                        onClick={handleFactoryReset}
                        className="flex items-center gap-2 px-8 py-4 bg-gray-600 hover:bg-gray-700 text-white rounded-lg font-bold shadow-lg transition-all active:scale-95 w-full justify-center md:w-auto"
                    >
                        <RotateCcw className="w-5 h-5" />
                        REINICIAR NAVEGADOR (Manteniendo Diseño)
                    </button>
                    <p className="text-gray-500 text-xs mt-2">
                        * Haz esto SOLO después de haber restaurado los datos con éxito.
                    </p>
                </div>
                ...
                <div className="bg-green-50 p-6 rounded-xl border border-green-200 mt-6">
                    ...
                    <p className="text-green-600 text-xs mt-2">
                        * Usa esto si hiciste "Factory Reset" y tu sistema está vacío.
                    </p>
                </div>
            </div>
        </div>
